...
| **`grant-proposals`** | 🚧 in development ([spec](./skills/grant-proposals/SPEC.md)) | A grant-writing assistant for **DFG · ERC/Horizon · NIH · industry** funders. It works from *your* real science (it never fabricates) — interviews you, ingests your call/RFA + papers, grounds every claim in real literature via **Paperclip**, **pressure-tests the idea before you write a word** (is it actually novel/fundable?), drafts to the agency's best practices, machine-checks limits + citations, and writes it into **Google Docs** with figures + a Gantt from `research-visuals`. Concierge setup walks anyone through it. |
